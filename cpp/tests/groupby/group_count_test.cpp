...
//     test_single_agg(keys, vals, expect_keys, expect_vals, std::move(agg1),
...
//   auto const expect_keys = fixed_width_column_wrapper<K>{1, 2, 3};
//   auto const expect_vals = fp_wrapper{{9, 19, 17}, scale};
...
TEST_F(FixedPointTestBothReps, GroupByMin)
{
  using namespace numeric;
  using decimalXX  = decimal32;
  using RepType    = cudf::device_storage_type_t<decimalXX>;
  using fp_wrapper = cudf::test::fixed_point_column_wrapper<RepType>;
