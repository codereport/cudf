...
struct find_index_fn {
  template <typename Element,
            std::enable_if_t<not std::is_same<Element, dictionary32>::value and
                             ...
                             not std::is_same<Element, numeric::decimal32>::value>* = nullptr>
  ...
  template <typename Element,
            std::enable_if_t<std::is_same<Element, numeric::decimal32>::value>* = nullptr>
  std::unique_ptr<numeric_scalar<int32_t>> operator()(dictionary_column_view const& input,
                                                      scalar const& key,
                                                      rmm::mr::device_memory_resource* mr,
                                                      cudaStream_t stream) const
  {
    CUDF_FAIL("decimal32 column cannot be the keys column of a dictionary");
  }
};
